...
bolt.register_task(
    "lcov", ["clear-pyc", "mkdir", "mkdir.test", "shell.pytest.coverage"]
)
bolt.register_task("pack", ["setup", "setup.egg-info"])

# CI/CD tasks
bolt.register_task(
    "run-unit-tests", ["clear-pyc", "mkdir", "mkdir.test", "shell.pytest.ci"]
)
...
config = {
    "pip": {"command": "install", "options": {"r": "./requirements.txt"}},
    "delete-pyc": {
        "sourcedir": SRC_DIR,
        "recursive": True,
        "test-pyc": {
    ...
    "conttest": {"task": "ut"},
    "mkdir": {"directory": OUTPUT_DIR, "test": {"directory": TEST_OUTPUT_DIR}},
    "setup": {"command": "bdist_wheel", "egg-info": {"command": "egg_info"}},
    "coverage": {
        "task": "shell.pytest",
        "include": ["bolt"],
