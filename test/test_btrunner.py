...
class TestTaskRunner(unittest.TestCase):
    ...
    def setUpRegistry(self):
        ...
        self.registry.register_task(
            "failing_task_throug_return_code", self.failing_task_with_error_code
        )
        self.registry.register_task(
            "multi_task_with_failures", ["task_1", "failing_task", "task_2"]
        )
        self.tear_down_task = TearDownTask()
        self.registry.register_task("has_tear_down", self.tear_down_task)
        self.registry.register_task(
            "partially_executed", ["failing_task", "has_tear_down"]
        )

    def setUpConfig(self):
        self.config = {"configurable": {"param": "value"}}
    ...
    def test_continues_on_error_if_specified(self):
        ...
        self.subject = runner.TaskRunner(
            self.config_mgr, self.registry, continue_on_error
        )
